...
3. Wil je de testdata van deze demo gebruiken: download dan de [Beeldbank Zeeland](http://erfgoedenlocatie.cloud.tilaa.com/erfgeoviewer-demo-data/bbz/), beschikbaar als open data, onder licentie [CC-BY-SA](https://creativecommons.org/licenses/by-sa/3.0/nl/), (c) [Zeeuwse Bibliotheek](http://www.zeeuwsebibliotheek.nl/).
De tweede dataset betreft het [molenbestand](http://erfgoedenlocatie.cloud.tilaa.com/erfgeoviewer-demo-data/molens/), (c) [Rijksdienst voor het Cultureel Erfgoed](http://www.cultureelerfgoed.nl). Beide datasets zijn getransformeerd naar Linked Open Data en beschikbaar gesteld door [MetaMatter](http://metamatter.nl), met dank aan Roland Cornelissen.
4. Configureer de erfgeoviewer javascript applicatie zodat deze verwijst naar de virtuoso server: pas hiervoor de config.js aan.
5. Configureer de erfgeoviewer javascript applicatie zodat de SPARQL queries de data correct bevragen die in je Virtuoso-installatie zitten: pas hiervoor de config.js aan. 
6. Het kan nodig zijn CORS headers te bieden voor je virtuoso server: volg hiervoor [deze aanwijzingen](http://virtuoso.openlinksw.com/dataspace/doc/dav/wiki/Main/VirtTipsAndTricksCORsEnableSPARQLURLs).
7. Deploy de javascript op een locatie die door Apache2 web server wordt geserveerd.
